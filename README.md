...
The slim platform archives are temporary workflow artifacts used to assemble and test the NuGet package. They are not published as GitHub release assets; GitHub releases continue to contain only the full platform bundles.
...
`SlangDxcBundle.Toolchain` is published by GitHub Actions through [NuGet trusted publishing](https://learn.microsoft.com/nuget/nuget-org/trusted-publishing), using a short-lived credential issued for each publication run. The repository stores no long-lived NuGet API key.
